...
const algorithm_module = @import("algorithm.zig");
const composition = @import("../transform/composition.zig");
const domain = @import("../core/domain.zig");
const exact = @import("../core/exact.zig");
const limits = @import("../core/limits.zig");
const multi = @import("../transform/multi.zig");
const parser = @import("../parse/parser.zig");
const polynomial = @import("../algebra/polynomial.zig");
const rational_function = @import("../algebra/rational_function.zig");
...
pub fn Factorization(
    ...
    return struct {
        ...
        algorithm: algorithm_module.SolveAlgorithm,
...
pub fn factorSystem(
    ...
    var condition_storage: [limits.symbolic_conditions]solution.Condition = undefined;
...
fn appendSolutionCondition(
    storage: *[limits.symbolic_conditions]solution.Condition,
    len: *usize,
...
pub fn solveSystem(
    ...
    if (algorithm == .polynomial) {
        ...
        return @import("polynomial.zig").solve(
            problem.equations[0],
            problem.unknowns[0],
            problem.domain,
        );
...
fn solveData(
    ...
    comptime algorithm: algorithm_module.SolveAlgorithm,
) solution.SolutionSet(N) {
...
fn parseAlgorithm(comptime value: anytype) algorithm_module.SolveAlgorithm {
    const name = @tagName(value);
    inline for (@typeInfo(algorithm_module.SolveAlgorithm).@"enum".fields) |field| {
