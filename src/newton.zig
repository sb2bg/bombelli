...
pub fn compileSystem(
    ...
    if (!@hasField(Options, "algorithm") or
        !std.mem.eql(u8, @tagName(options.algorithm), "newton"))
    {
        @compileError("Bombelli generated system compilation requires '.algorithm = .newton'");
    }
    if (!@hasField(Options, "jacobian") or
        !std.mem.eql(u8, @tagName(options.jacobian), "symbolic"))
    {
        @compileError("Bombelli Newton solver requires '.jacobian = .symbolic'");
    }
    if (!@hasField(Options, "tolerance")) {
        @compileError("Bombelli Newton solver options require '.tolerance'");
    }
