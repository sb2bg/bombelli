...
pub fn HybridIntegral(comptime order: usize) type {
    return struct {
        ...
        variable: []const u8,
        bounds: ?integration.IntegralBounds,
...
pub fn compilePartial(
    comptime partial: integration.PartialIntegral,
    comptime options: anytype,
) HybridIntegral(options.order) {
    if (!@hasField(@TypeOf(options), "rule")) {
        @compileError("Bombelli hybrid integration options require '.rule'");
    }
    if (!@hasField(@TypeOf(options), "order")) {
        @compileError("Bombelli hybrid integration options require '.order'");
    }
